...
export function DashboardOverview() {
  ...
  const { setShowAssignmentModal } = useData();

  const [showReports, setShowReports] = useState(false);

  const [dashboardStudents, setDashboardStudents] = useState<any[]>([]);
  const [dashboardBatches, setDashboardBatches] = useState<any[]>([]);
  const [dashboardAssignments, setDashboardAssignments] = useState<any[]>([]);
  ...
  const fetchJsonArray = async (url: string, options: RequestInit = {}) => {
    try {
      const res = await fetch(url, options);
      const data = await res.json();

      if (!res.ok) {
        console.error("Dashboard fetch failed:", url, data);
        return [];
      }
  ...
  const loadDashboardData = async () => {
    ...
    const [
      ...
      fetchJsonArray(`${API_URL}/admin/students`, {
        headers: authHeaders,
      }),
      fetchJsonArray(`${API_URL}/batches`),
      fetchJsonArray(`${API_URL}/payments`),
      fetchJsonArray(`${API_URL}/assignments`, {
        headers: authHeaders,
      }),
      fetchJsonArray(`${API_URL}/assignments/announcements`, {
        headers: authHeaders,
      }),
    ]);
    ...
    setDashboardBatches(batchesData);
    setDashboardPayments(mappedPayments);
    setDashboardAssignments(mappedAssignments);
    setDashboardAnnouncements(mappedAnnouncements);
  ...
  const getAssignmentSortDate = (assignment: any) => {
    return (
  ...
  const getAssignmentDueDate = (assignment: any) => {
    return assignment?.dueDate || assignment?.deadline || assignment?.date || "";
  };

  const recentActivity = [
    ...dashboardAnnouncements.map((announcement: any) => {
      const activityDate = getAnnouncementDate(announcement);

      return {
        id: `announcement-${announcement.id}`,
        type: "announcement",
        action: "Announcement posted",
        target: announcement.title || "Untitled announcement",
        dateText: formatDate(activityDate),
        sortTime: new Date(activityDate || Date.now()).getTime(),
      };
    }),

    ...dashboardAssignments.map((assignment: any) => {
      const sortDate = getAssignmentSortDate(assignment);
      ...
      return {
        ...
        target: assignment.title || "Untitled assignment",
        dateText: formatDate(dueDate),
        sortTime: new Date(sortDate || Date.now()).getTime(),
      };
    }),
  ]
  ...
  const paidPayments = dashboardPayments.filter((p) => p.status === "Paid");
  const unpaidPayments = dashboardPayments.filter((p) => p.status === "Unpaid");
  ...
  const gradingAssignments = dashboardAssignments.filter(
    (a) => a.status === "Grading"
  ).length;

  const activeAssignments = dashboardAssignments.filter(
    (a) => a.status === "Active"
  ).length;

  const upcomingAssignments = dashboardAssignments.filter(
    (a) => a.status === "Upcoming"
  ).length;
  ...
  const summaryData = [
    {
      ...
      value: isDashboardLoading ? "..." : dashboardStudents.length.toString(),
      icon: Users,
      bgColor: "bg-blue-50",
    ...
    {
      title: "Total Batches",
      value: isDashboardLoading ? "..." : dashboardBatches.length.toString(),
      icon: BookOpen,
    ...
    },
    {
      ...
      value: isDashboardLoading ? "..." : dashboardAssignments.length.toString(),
      icon: FileText,
      bgColor: "bg-green-50",
    ...
    },
    {
      ...
      value: isDashboardLoading
        ? "..."
        : `Rs. ${totalRevenue.toLocaleString(undefined, {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {showReports && (
        <div
          ...
          onClick={(e) => e.target === e.currentTarget && setShowReports(false)}
        >
                      ...
                      {dashboardBatches.map((batch) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-sm font-bold bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full">
                              {count} Students
                            </span>
                          </div>
                        );
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-full">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="text-lg font-medium text-blue-600 hover:text-blue-700 flex items-center gap-1 group"
              >
                View all
                <ArrowRight
                  size={16}
            ...
            <div className="space-y-6">
              {recentActivity.length > 0 ? (
                recentActivity.map((activity) => {
                  const ActivityIcon =
                    activity.type === "announcement" ? Bell : FileText;
        ...
        <div>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-full">
            ...
            <div className="flex flex-col gap-4">
              <button
                onClick={() =>
                  router.push("/admin/dashboard/students?action=add")
                }
                className="w-full flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-blue-50 border border-gray-100 group transition-all duration-200"
              >
                <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-green-50 border border-gray-100 group transition-all duration-200"
              >
                <div className="flex items-center gap-3">
              ...
              <button
                onClick={() =>
                  router.push("/admin/dashboard/payments?action=add")
                }
                className="w-full flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-purple-50 border border-gray-100 group transition-all duration-200"
              >
                <div className="flex items-center gap-3">
